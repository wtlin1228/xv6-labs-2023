...
e1000_transmit(struct mbuf *m)
{
  ...
  // Use mbuffree() to free the last mbuf that was transmitted from 
  // that descriptor (if there was one)
  if (tx_mbufs[tx_ring_index] != 0) {
    mbuffree(tx_mbufs[tx_ring_index]);
  }
  // Then fill in the descriptor. m->head points to the packet's content 
  // in memory, and m->len is the packet length.
...
e1000_recv(void)
{
  ...
  // Then check if a new packet is available by checking for the E1000_RXD_STAT_DD
  // bit in the status portion of the descriptor. If not, stop.
  while (rx_ring[rx_ring_index].status & E1000_RXD_STAT_DD) {
    ...
    // given to net_rx(). Program its data pointer (m->head) into the descriptor. 
    ...
    // Finally, update the E1000_RDT register to be the index of the last 
    // ring descriptor processed.
    regs[E1000_RDT] = rx_ring_index;
    rx_ring_index += 1;
    rx_ring_index %= RX_RING_SIZE;
  }
